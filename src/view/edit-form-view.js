...
export default class EditFormView extends AbstractStatefulView {
  ...
  #typeToggleHandler = (e) => {
    ...
    this.updateElement({
      type: e.target.value,
      offers: [],
      availableOffers: OFFERS_BY_TYPE.find(
  ...
  #setInnerHandlers = () => {
    ...
    Array.from(this.element.querySelectorAll('.event__type-input')).forEach(
      (typeElement) =>
        typeElement.addEventListener('click', this.#typeToggleHandler)
    );
    Array.from(this.element.querySelectorAll('.event__offer-checkbox')).forEach(
      (offerElement) =>
        offerElement.addEventListener('click', this.#offerToggleHandler)
    );
  };
